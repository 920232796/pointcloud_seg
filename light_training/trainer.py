...
class Trainer:
    ...
    def validate(self):
        ...
        if self.val_loader is not None :
            ...
            # for i in tqdm(range(len(self.val_loader)), total=len(self.val_loader)):
                ...
                with torch.no_grad():
                    with torch.autocast("cuda", enabled=True) if (self.ddp or 'cuda' in self.device) else dummy_context():
                        val_out = self.validation_step(batch)
                        assert val_out is not None
                        assert (type(val_out) is list or type(val_out) is tuple)
            ...
            ## 先汇总结果。
            ## outputs 里面包含多个metric 汇总的时候 分开汇总
            if self.ddp:
                torch.distributed.barrier()
                val_outputs_merge = []
    ...
    def train(self,
        ...
        if self.model is not None:
            ...
            if self.local_rank == 0:
                ...
                print(f"trainable_num parameters is {trainable_num * 4 / 1000 / 1000}M")
